/**
 * Tests for SecurityApartment building
 *
...
describe('SecurityApartment Building', () => {
  let apartment: SecurityApartment;
  let player: IPlayerState;
  let game: IGame;
  const testPosition = new Position(4, 3);

  beforeEach(() => {
    apartment = new SecurityApartment('security-apt-1', 'Secure Towers', testPosition);
  ...
  describe('Constructor and Properties', () => {
    it('should initialize with correct properties', () => {
      ...
      expect(apartment.name).toBe('Secure Towers');
      expect(apartment.description).toContain('Secure apartment');
      expect(apartment.description).toContain('$445/week');
    ...
    it('should have higher rent than low cost apartment', () => {
      expect(SecurityApartment.WEEKLY_RENT).toBeGreaterThan(305);
    });
  });
  ...
  describe('Building Access', () => {
    ...
    it('should allow entry if player rents any security apartment', () => {
      player = createMockPlayer({ rentedHome: 'security-apt-2' });
      expect(apartment.canEnter(player)).toBe(true);
    });
  ...
  describe('Available Actions - Not Renter', () => {
    ...
    it('should only offer exit if inside but not renter', () => {
      player = createMockPlayer({
        currentBuilding: 'security-apt-1',
        rentedHome: null,
      });
      const actions = apartment.getAvailableActions(player, game);
  ...
  describe('Relax Action', () => {
    ...
    it('should restore health and happiness with enhanced home bonus', () => {
      const result = relaxAction.execute(player, game);

      expect(result.success).toBe(true);
      expect(result.message).toContain('Relaxed');
      expect(result.message).toContain('enhanced home bonus');
      ...
      // Should restore 15 health and 20 happiness (better than low cost)
      expect(healthChange.value).toBe(Math.min(100, player.health + 15));
      expect(happinessChange.value).toBe(Math.min(100, player.happiness + 20));
    });

    it('should provide better bonuses than low cost apartment', () => {
      const result = relaxAction.execute(player, game);

      const healthChange = result.stateChanges.find((c: any) => c.measure === 'HEALTH');
      const happinessChange = result.stateChanges.find((c: any) => c.measure === 'HAPPINESS');

      const healthGain = healthChange.value - player.health;
      const happinessGain = happinessChange.value - player.happiness;

      // Security apartment should give better bonuses
      expect(healthGain).toBeGreaterThanOrEqual(14); // At least 14-15
      expect(happinessGain).toBeGreaterThanOrEqual(20); // At least 20
    });
  ...
  describe('Pay Rent Action', () => {
    ...
    it('should successfully pay weekly rent', () => {
      ...
      expect(cashChange.value).toBe(2000 - SecurityApartment.WEEKLY_RENT);
    });

    it('should charge higher rent than low cost apartment', () => {
      const result = payRentAction.execute(player, game);

      const cashChange = result.stateChanges.find((c: any) => c.type === 'cash');
      const rentPaid = player.cash - cashChange.value;

      expect(rentPaid).toBe(SecurityApartment.WEEKLY_RENT);
      expect(rentPaid).toBeGreaterThan(305);
    });
  ...
  describe('Comparison with LowCostApartment', () => {
    it('should have higher rent cost', () => {
      expect(SecurityApartment.WEEKLY_RENT).toBeGreaterThan(305);
      expect(SecurityApartment.WEEKLY_RENT).toBe(445);
    });

    it('should provide better relax bonuses', () => {
      player = createMockPlayer({
        currentBuilding: 'security-apt-1',
        rentedHome: 'security-apt-1',
        health: 50,
      ...
      const actions = apartment.getAvailableActions(player, game);
      const relaxAction = actions.find(a => a.type === ActionType.RELAX);
      const result = relaxAction?.execute(player, game);
      ...
      // Security should give at least 15 health and 20 happiness
      // Low cost gives 10 health and 15 happiness
      expect(healthGain).toBeGreaterThanOrEqual(15);
      expect(happinessGain).toBeGreaterThanOrEqual(20);
    });
  });
});
